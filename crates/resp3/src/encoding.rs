...
pub fn array(elements: Vec<RESPValue>) -> RESPValue {
...
impl CommandEncoding for &'static str {
    fn encode(self, dest: &mut Vec<Bytes>) {
        Bytes::from_static(self.as_bytes()).encode(dest);
    }
}

impl CommandEncoding for i64 {
    fn encode(self, dest: &mut Vec<Bytes>) {
...
impl<T: CommandEncoding> CommandEncoding for Option<T> {
    fn encode(self, dest: &mut Vec<Bytes>) {
...
macro_rules! tuple_impls {
    ...
    (@impl $( $idx:tt $T:ident ),+ ) => {
        impl<$( $T: CommandEncoding ),+> CommandEncoding for ( $( $T, )+ ) {
            #[inline]
            fn encode(self, dest: &mut Vec<Bytes>) {
